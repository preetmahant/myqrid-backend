...
function normalizeAnalytics(analytics) {
  ...
  return {
    ...
    total_scans: Number(safe.total_scans || 0),
    total_clicks: Number(safe.total_clicks || 0),
    total_leads: Number(safe.total_leads || 0),
    link_clicks: safe.link_clicks && typeof safe.link_clicks === "object" ? safe.link_clicks : {},
    device_counts: safe.device_counts && typeof safe.device_counts === "object" ? safe.device_counts : {}
  };
}
...
function leadActions() {
  return new Set(["whatsapp_click", "call_click", "email_click", "website_click", "save_contact", "copy_link", "share_profile", "social_click"]);
}

const allowedThemes = new Set(["dark-mesh", "purple-nebula", "neon-city", "glass", "light-pearl", "royal-velvet"]);
...
function parseLinks(body) {
  const raw = body.links_json || body.links || [];
  let links = raw;
  if (typeof raw === "string") {
    try {
      links = raw.trim() ? JSON.parse(raw) : [];
    } catch {
      links = [];
    }
  }
  ...
  return links
    .slice(0, 40)
    .map((link, index) => {
      const type = allowedLinkTypes.has(cleanText(link.type).toLowerCase()) ? cleanText(link.type).toLowerCase() : "custom";
      const label = cleanText(link.label || type, type).slice(0, 80);
      const value = cleanText(link.value || link.url || link.href).slice(0, 300);
      const enabled = link.enabled === false || link.visible === false ? false : true;
      return { id: cleanText(link.id || `${type}-${index + 1}`).replace(/[^a-z0-9_-]/gi, "").slice(0, 50) || `${type}-${index + 1}`, type, label, value, enabled, order: index };
    })
...
function validateProfileInput(body) {
  ...
  const requestedTheme = cleanText(body.theme || "dark-mesh").toLowerCase();
  const theme = allowedThemes.has(requestedTheme) ? requestedTheme : "dark-mesh";
  const links = parseLinks(body);
  const emergencyName = cleanText(body.emergency_name || "Emergency contact");
  const emergencyPhone = normalizePhone(body.emergency_phone || body.phone);
  ...
  return { value: { name, phone, whatsapp, email, bio, image_url: imageUrl, website, theme, links, emergency_name: emergencyName, emergency_phone: emergencyPhone } };
}
...
async function logEvent(event) {
  ...
  memoryStore.events.unshift(payload);
  memoryStore.events = memoryStore.events.slice(0, 500);
}

async function recentEvents(limit = 50) {
  if (db) {
    const snapshot = await db.collection("scan_logs").orderBy("created_at", "desc").limit(limit).get();
...
async function createProfileHandler(req, res) {
  try {
    ...
    await logEvent({ username, tag_slug: tagSlug, action: "profile_created", storage: storeMode(), device_type: deviceType(req.get("user-agent")), user_agent: req.get("user-agent") || "" });
...
async function getProfileHandler(req, res) {
  try {
    ...
    const seenDevice = deviceType(req.get("user-agent"));
    analytics.total_views += 1;
    analytics.last_seen = new Date().toISOString();
    analytics.device_counts[seenDevice] = Number(analytics.device_counts[seenDevice] || 0) + 1;
...
async function getTagHandler(req, res) {
  try {
    ...
    const analytics = normalizeAnalytics(profile.analytics);
    const scanDevice = deviceType(req.get("user-agent"));
    analytics.total_scans += 1;
    analytics.last_scan_at = new Date().toISOString();
    analytics.device_counts[scanDevice] = Number(analytics.device_counts[scanDevice] || 0) + 1;
...
app.put("/api/profiles/:username", async (req, res) => {
  try {
    ...
    const providedToken = req.get("x-edit-token") || req.body.edit_token;
    const adminPassword = req.get("x-admin-password") || req.body.admin_password;
    const adminAllowed = (process.env.ADMIN_PASSWORD || process.env.SETUP_SECRET) && adminPassword === (process.env.ADMIN_PASSWORD || process.env.SETUP_SECRET);
    if (!adminAllowed && providedToken !== existing.edit_token) {
      return res.status(401).json({ success: false, error: "Invalid edit token." });
    }
    ...
    await logEvent({ username, tag_slug: updated.tag_slug, action: "profile_edited", storage: storeMode(), device_type: deviceType(req.get("user-agent")), user_agent: req.get("user-agent") || "" });
    return res.json({ success: true, storage: storeMode(), profile: updated, vcard: buildVCard(updated) });
  } catch (err) {
    return res.status(500).json({ success: false, error: err.message });
  }
});
...
app.post("/api/track", async (req, res) => {
  try {
    ...
    const clickDevice = deviceType(req.get("user-agent"));
    const profile = username ? await getProfile(username) : null;

    if (profile) {
      const analytics = normalizeAnalytics(profile.analytics);
      analytics.total_clicks += 1;
      if (leadActions().has(action)) analytics.total_leads += 1;
      analytics.link_clicks[action] = Number(analytics.link_clicks[action] || 0) + 1;
      analytics.device_counts[clickDevice] = Number(analytics.device_counts[clickDevice] || 0) + 1;
      await updateProfile(username, { analytics, updated_at: new Date().toISOString() });
    }

    await logEvent({ username, action, storage: storeMode(), device_type: clickDevice, user_agent: req.get("user-agent") || "", metadata: req.body.metadata || {} });
...
app.get("/api/admin/profiles", async (req, res) => {
  try {
    ...
    const summary = profiles.reduce((totals, profile) => {
      const analytics = normalizeAnalytics(profile.analytics);
      totals.total_profiles += 1;
      totals.total_scans += analytics.total_scans;
      totals.total_leads += analytics.total_leads;
      totals.total_views += analytics.total_views;
      return totals;
    }, { total_profiles: 0, total_scans: 0, total_leads: 0, total_views: 0 });
    return res.json({ success: true, storage: storeMode(), summary, profiles, recent_activity: events });
  } catch (err) {
    return res.status(500).json({ success: false, error: err.message });
  }
});

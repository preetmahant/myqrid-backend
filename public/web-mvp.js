...
const state = { profile: null, adminPassword: sessionStorage.getItem("myqrid_admin_password") || "" };
...
function renderActionButtons(profile, mount) {
  mount.innerHTML = `
    <a class="mvp-primary" href="tel:${escapeHtml(profile.phone)}">Call</a>
    <a class="mvp-secondary" href="${escapeHtml(whatsappUrl(profile))}" target="_blank" rel="noreferrer">WhatsApp</a>
    <button class="mvp-secondary" id="saveVcf" type="button">Save Contact</button>
    <button class="mvp-secondary" id="copyLink" type="button">Copy profile link</button>
  `;
  mount.querySelector("a[href^='tel:']")?.addEventListener("click", () => track("call", profile.username));
  mount.querySelector("a[href^='https://wa.me']")?.addEventListener("click", () => track("whatsapp", profile.username));
  mount.querySelector("#saveVcf")?.addEventListener("click", () => downloadVcf(profile));
  mount.querySelector("#copyLink")?.addEventListener("click", () => copyText(profile.profile_url, "Profile link copied"));
}

async function track(action, username) {
  if (!username) return;
  try {
...
async function createProfile(event) {
  ...
  try {
    ...
    button.textContent = "Save profile + generate QR";
  }
}
...
async function loadTag(slug) {
  ...
  try {
    ...
    renderPublic(data.profile, "Tag QR", data.tag);
  } catch (err) {
    renderPublicError(err.message);
  }
}
...
function renderPublic(profile, label, tag = null) {
  ...
  $("#publicBadge").textContent = tag ? "myQRID tag scan" : "Verified myQRID";
...
async function adminLoad(password) {
  ...
  try {
    ...
    if (!data.profiles.length) {
      list.innerHTML = `<p class="mvp-muted">No profiles yet.</p>`;
      return;
    }
    list.innerHTML = data.profiles.map(profile => `
      <div class="mvp-list-row" data-username="${escapeHtml(profile.username)}">
        <div><strong>${escapeHtml(profile.name)}</strong><small>@${escapeHtml(profile.username)} · ${escapeHtml(profile.phone)}</small></div>
